...
from src.indexer.clients import AzureSearchClient
...
def test_azure_openai_connection(settings: Settings):
    """Tests connection to Azure OpenAI Embedding model."""
    print("\n--- Testing Azure OpenAI Connection ---")
    try:
        search_client = AzureSearchClient(settings)
        embedding = search_client.embedding_client.get_embedding("test")

        print(
            f"✅ Success: Correctly assembled endpoint '{settings.aoai_endpoint}' and got a response. Vector dimensions: {len(embedding)}"
        )
...
if __name__ == "__main__":
    ...
    # 3. 接続テストを実行
    test_azure_openai_connection(app_settings)
