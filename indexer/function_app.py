...
def indexer(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        if not file_name:
            return func.HttpResponse("File name could not be determined.", status_code=400)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            logging.info(f"Temporary file created at {temp_file_path}. Extracting text.")
        ...
        if not text:
            ...
            return func.HttpResponse(
                f"'{file_name}' からテキストを抽出できませんでした。対応していないファイル形式か、ファイルが空の可能性があります。",
                status_code=400
            )
        ...
        logging.info(f"Chunking complete. Indexing {len(chunks)} chunks to Azure Search.")
